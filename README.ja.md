...
ヒープ確保の **`Box<T>`**、可変長の **`Vec<T>`**、文字列スライス **`str`**、所有文字列 **`String`**、**`HashMap<K, V>`**。
...
struct/enum/tuple/array/`Vec` に再帰します。上のリストは全ノードが解放され、
リークも二重解放もありません。

### `Vec<T>`（可変長配列）

`Vec<T>` はヒープ上の動的配列です。`vec()` で空の Vec を作り（要素型は型注釈から
推論）、`push(v, x)` で末尾に追加し（足りなければ再確保して成長）、`len(v)` が
長さ、`v[i]` で要素を読み書きします（実行時境界チェックつき）。Vec はバッファを
所有するので自動で drop されます――生存している各要素を drop してからバッファを
解放します。
...
`push(v, x)` は `v` が可変な場所（`let mut`）であることを要求し、`x` を**ムーブ**で
取り込みます。`pop(v)` は末尾要素を取り除いて `Option<T>` で返し（空なら `None`）、
`clear(v)` は全要素を drop して長さを 0 に戻します（容量は保持）。どちらも可変な
`Vec` が必要です。

添字では非 `Copy` 要素をムーブ取り出しできません（`Vec<Box<_>>` での `let b = v[0]`
はエラー。スライスと同じ）――`Copy` 要素を読むか、借用するか、`pop` を使ってください
（`pop` こそ要素をムーブで取り出す方法で、長さを縮めるので Vec はその要素を二重に
drop しません）。

> **注意:** 文の位置で捨てられる所有ヒープ値（`box(x);`・`pop(v);`・所有値を返す
> 関数呼び出しの結果無視など）はその文末で解放され、借用する操作（`prints`/`len`/
> `push_str`、文字列演算子 `+`/`==` など、`str` 仮引数）に渡した所有一時値はその
> 操作後に解放され、`&self`/`&mut self` メソッドのレシーバにした所有一時値
> （`Type::new(…).m()`・`mk().m()`）は呼び出し後に解放されます。残る狭い制限は 1 つ:
> 一時値を添字アクセスした場合（`(a + b)[0]`）はまだリークします（先に `let` に
> 束縛してください）。また要素の借用（`&v[i]`）を `push`（再確保が起こりうる）を
> またいで保持してはいけません。どちらもメモリ安全で（最悪リークするだけ・二重解放
> なし）、借用チェッカ（今後）が解消します。

### 文字列（`str`）

文字列リテラル `"..."` の型は **`str`** ＝ UTF-8 バイト列の読み取り専用ビュー
（`{ptr, len}`、スライスと同形）。静的データを指すので `str` は `Copy` で、
ヒープ確保も drop も不要です。エスケープは `\n \t \r \0 \\ \"`。

```
prints("Hello\n");          # prints(s): str を出力（末尾改行なし）
let s: str = "world";
len(s);                     # => 5   （バイト長。UTF-8 はバイト単位）
s[0];                       # => 'w' を u8 で（境界チェックつきバイトアクセス）
```

`str` は変数・引数・戻り値・フィールド・ペイロードなどに使えます。

実行時に組み立てたい文字列には、所有・可変な **`String`**（ヒープ上の
`{ptr, len, cap}` ≒ `Vec<u8>`）を使います。`string(s)` は `str` をヒープに
コピーし、`push_str(s, t)` は `str` を追記（必要なら再確保）、`String` は
スコープを抜けるとバッファが解放されます。`prints`・`len`・`s[i]` は `str` と
`String` の両方を受け付けます。
...
`push_str` には可変な `String` が必要です（`str` は読み取り専用なので
`s[i] = …` は不可）。

文字列は `== != < <= > >=` で**バイト辞書順**比較できます。`str` と `String` は
自由に混在でき、両辺とも借用されます（比較はオペランドを消費しません）:

```
"abc" < "abd";              # => true
"ab"  < "abc";              # => true   （接頭辞が先）
let s: String = string("hi");
s == "hi";                  # => true   （比較後も s は使える）
```

`+` で文字列を連結できます。`str`/`String` を自由に混在でき、結果は新しい所有
`String` です。両辺は借用される（変数は連結後も使える）うえ、所有 String の
一時オペランドは自動で解放されるので、チェーンや再代入でリークしません:

```
let s: String = "Hello" + ", " + "world";   # チェーン。中間 String も解放される
...
ハッシュマップ（オープンアドレッシング・自動拡張）。キー `K` は整数・`bool`・`str`、
値 `V` は `Copy` 型（`get` が値のコピーを返すため）。`str` キーはマップが所有コピーを
持ち、drop 時に解放されます。
...
`insert(m, …)` は可変な `HashMap` が必要です。`insert`/`get`/`contains` はマップと
キーを借用します（ムーブしません）。`str` キーの位置には `String` も渡せます
（挿入時はコピー、検索時は借用）。`String` を**値**にすることはまだできません
（値は `Copy` 型のみ）。
...
- `prints(s: str | String)` … 文字列のバイトを出力（末尾改行なし）
- `len(s: &[T] | Vec<T> | str | String) -> i64` … 長さ（スライス・`Vec`・文字列。バイト数）
- `box(x: T) -> Box<T>` … x をヒープへ移す
- `vec() -> Vec<T>` … 空の可変長配列を作る（要素型は文脈から推論）
- `push(v: Vec<T>, x: T)` … 可変な `Vec` の末尾に `x` を追加（必要なら成長）
- `pop(v: Vec<T>) -> Option<T>` … 末尾要素を取り除いて返す（空なら `None`）
- `clear(v: Vec<T>)` … 全要素を drop して長さ 0 に（容量は保持）
- `string(s: str) -> String` … `str` を所有ヒープ `String` にコピー
- `push_str(s: String, t: str)` … 可変な `String` に `str` を追記
- `hashmap() -> HashMap<K, V>` … 空のハッシュマップを作る（型は文脈から）
- `insert(m: HashMap<K,V>, k: K, v: V)` … 可変なマップに挿入/上書き
- `get(m: HashMap<K,V>, k: K) -> Option<V>` … キーで引く
- `contains(m: HashMap<K,V>, k: K) -> bool` … キーの有無
