...
ヒープ確保の **`Box<T>`**、可変長の **`Vec<T>`**、文字列スライス **`str`**。
...
`str` は変数・引数・戻り値・フィールド・ペイロードなどに使えます。等価比較
（`==`）・連結・所有/可変な `String` はまだ未実装です。

### 組み込み関数
- `printi(x: i64)` … 整数を 1 行で表示
- `printd(x: f64)` … 浮動小数点を 1 行で表示
- `putchard(x: i64)` … 文字コード x の 1 文字を出力（例: `putchard(10)` で改行）
- `prints(s: str)` … 文字列のバイトを出力（末尾改行なし）
- `len(s: &[T] | Vec<T> | str) -> i64` … スライス・`Vec`・`str` の長さ（str はバイト数）
