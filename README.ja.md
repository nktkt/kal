...
> 関数呼び出しの結果を無視した場合など）は、その文の終わりで解放されます。残る
> 制限が 1 つ: より大きな式の中で作られて**借用だけ**される一時値――例えば
> `len(make_vec())` のように、文の値そのものではない一時値――はまだリークします。
> 先に `let` に束縛してください。別件として、要素の借用（`&v[i]`）を `push`（再確保
> が起こりうる）をまたいで保持してはいけません（借用チェッカが今後強制）。残るどちらも
> メモリ安全です（最悪リークするだけで、二重解放はしません）。
