...
> **注意:** 名前に束縛されずに捨てられる所有ヒープ値（`Box`/`Vec` の一時値）は
> まだ解放されません――`let` に束縛すれば drop されます。また要素の借用
> （`&v[i]`）を `push`（再確保が起こりうる）をまたいで保持してはいけません。
> 借用チェッカ（今後の課題）がこれを強制します。どちらもメモリ安全です
> （一時値はリークするだけで、二重解放はしません）。
