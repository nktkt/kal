...
let n: i64 = 42 in get(&n);           # => 42   (let は `: T` 注釈に対応)

fn normX(p: &Point) -> f64 = (*p).x;  # 構造体を参照渡し
```

`&x` / `&mut x` は場所（変数・フィールド等）を参照として借用し、`*p` で参照外しします。
ローカルはメモリ常駐（アドレスを取れる）で、`-O` で再びレジスタへ昇格します。借用チェック
（ムーブ意味論・エイリアス規則・ライフタイム）は次フェーズ（[ROADMAP.md](ROADMAP.md)
Phase 3）。現状、参照は読み取り専用です。
