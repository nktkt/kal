...
> 関数呼び出しの結果を無視した場合など）はその文の終わりで解放され、借用する操作
> （`prints`/`len`/`push_str`、文字列演算子 `+`/`==` など、`str` 仮引数）に渡した
> 所有一時値はその操作の後に解放されます。残る狭い制限は 2 つ: 一時値を添字
> アクセスやメソッドのレシーバにした場合（`(a + b)[0]`・`mk().m()`）はまだリーク
> します（先に `let` に束縛してください）。また要素の借用（`&v[i]`）を `push`
> （再確保が起こりうる）をまたいで保持してはいけません。どちらもメモリ安全で
> （最悪リークするだけ・二重解放なし）、借用チェッカ（今後）が解消します。
