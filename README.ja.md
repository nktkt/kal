...
ユーザー定義の **`struct`**・**`enum`**（代数的データ型。**ジェネリック**可――
`enum Name<T> { … }`、`Option<T>` / `Result<T, E>` は組み込み）・タプル `(T, U, …)`・
...
enum は型に対して**ジェネリック**にできます: `enum Name<T, …> { … }`。具体化ごとに
*単態化*され（使用される型引数の組み合わせごとに別の型を生成）、型引数 `Name<T1, …>` は
構築時にペイロードと期待型から**推論**されます（turbofish 不要）。
...
注釈か型の決まった文脈が必要）。総称**関数**と総称**struct** は未実装です
